...
namespace LabelGenGUI.Generators
{
    [Generator]
    public class StaticViewLocatorGenerator : ISourceGenerator
    {
        private const string StaticViewLocatorAttributeDisplayString = "LabelGenGUI.StaticViewLocatorAttribute";
        ...
        private class SyntaxReceiver : ISyntaxContextReceiver
        {
            ...
            public void OnVisitSyntaxNode(GeneratorSyntaxContext context)
            {
                if (context.Node is ClassDeclarationSyntax classDeclarationSyntax)
                {
                    ...
                    {
                        NamedTypeSymbolLocators.Add((INamedTypeSymbol)namedTypeSymbol);
                    }
                    else if (namedTypeSymbol.Name.EndsWith(ViewModelSuffix))
                    {
                        if (!namedTypeSymbol.IsAbstract)
                        {
                            NamedTypeSymbolViewModels.Add((INamedTypeSymbol)namedTypeSymbol);
                        }
                    }
                }
            }
